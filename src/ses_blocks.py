...
from ses import _compute_triple_probe_centers, _probe_centers_accessible_from_exterior
...
class SESExteriorContext:
    ...
    Attributes:
        ...
        grid_spacing: Optional spacing override for the flood-fill based
            exterior test in :func:`ses._probe_centers_accessible_from_exterior`.
...
class SESAnalyticSamples:
    ...
    Attributes:
        ...
        block_indices: Row index of the source block within the corresponding
            block tensor in :class:`SESAnalyticBlocks`.
    ...
    blocks: Optional[SESAnalyticBlocks] = None


def build_ses_exterior_context(
    atom_coords: torch.Tensor,
    atom_radii: torch.Tensor,
    probe_radius: float,
    *,
    ...
    Returns:
        A :class:`SESExteriorContext` with normalized dtype/device and cached
        expanded radii.
...
def build_analytic_ses_blocks(
    ...
    *,
    ...
    context = build_ses_exterior_context(
        atom_coords,
        atom_radii,
        probe_radius,
        grid_spacing=grid_spacing,
    ...
    with torch.no_grad():
        ...
        atom_indices = extract_atom_contact_blocks(
            context,
            filter_samples=atom_filter_samples,
        )
        pair_indices = extract_pair_torus_blocks(
            context,
            filter_samples=pair_filter_samples,
        )
        probe_candidate_pair_indices = _candidate_pair_indices(context)
        probe_blocks = extract_probe_sphere_blocks(
            context,
            pair_indices=probe_candidate_pair_indices,
...
def extract_atom_contact_blocks(
    context: SESExteriorContext,
    *,
    ...
    centers = (
        context.atom_coords[:, None, :]
        + context.expanded_radii[:, None, None] * directions[None, :, :]
    )
    flat_centers = centers.reshape(-1, 3)
    exterior = context.centers_exterior(flat_centers).reshape(
        context.num_atoms,
        filter_samples,
    )
    return exterior.any(dim=-1).nonzero(as_tuple=False).reshape(-1)


def extract_pair_torus_blocks(
    context: SESExteriorContext,
    *,
    ...
    radial_dirs = (
        torch.cos(angles).view(1, -1, 1) * basis_u[:, None, :]
        + torch.sin(angles).view(1, -1, 1) * basis_v[:, None, :]
    )
    probe_centers = centers[:, None, :] + circle_radii[:, None, None] * radial_dirs
    exterior = context.centers_exterior(probe_centers.reshape(-1, 3)).reshape(
        pair_indices.shape[0],
        filter_samples,
    )
    return pair_indices[exterior.any(dim=-1)]
...
class _ProbeBlocks:
    ...
    This mirrors the fixed-probe fields in :class:`SESAnalyticBlocks` without
...
def extract_probe_sphere_blocks(
    context: SESExteriorContext,
    *,
...
def sample_atom_contact_blocks(
    context: SESExteriorContext,
    atom_indices: torch.Tensor,
    *,
    point_area: float,
    ...
    Args:
        context: Prepared molecule geometry.
        atom_indices: Atom indices returned by :func:`extract_atom_contact_blocks`.
    ...
    Returns:
        :class:`SESAnalyticSamples` whose support width is one.  The sampled
...
def sample_pair_torus_blocks(
    ...
    *,
    ...
    Args:
        context: Prepared molecule geometry.
        pair_indices: Atom pairs returned by :func:`extract_pair_torus_blocks`.
        point_area: Target surface area represented by one returned point.
        oversample_factor: Multiplier applied before exterior clipping.

    Returns:
        :class:`SESAnalyticSamples` whose support width is two.  Support weights
...
def sample_probe_sphere_blocks(
    context: SESExteriorContext,
    blocks: SESAnalyticBlocks,
    *,
    point_area: float,
    ...
    Returns:
        :class:`SESAnalyticSamples` whose support width matches
...
def sample_analytic_ses_points(
    atom_coords: torch.Tensor,
    atom_radii: torch.Tensor,
    probe_radius: float,
    *,
    ...
    """Generate SES points from explicit atom, pair and fixed-probe blocks.

    This is the high-level entry point for analytic SES sampling.  When
    ``blocks`` is not supplied, topology is discovered first.  The three block
    families are then sampled independently and concatenated with compatible
    sparse support metadata.
    ...
    Args:
        ...
        include_atom_weights: If true, also return a dense ``(points, atoms)``
            multi-hot/weighted atom assignment matrix. The default sparse
            ``support_*`` fields are much cheaper for large molecules.
        blocks: Optional precomputed blocks from :func:`build_analytic_ses_blocks`.
    ...
    Returns:
        :class:`SESAnalyticSamples` with point coordinates and source-atom
        assignments.
    """
...
def _packed_fibonacci_directions(
    ...
    directions = [
        _fibonacci_unit_vectors(int(count.item()), dtype=dtype, device=device)
        for count in counts
        if int(count.item()) > 0
    ]
    if not directions:
...
def _candidate_pair_indices(context: SESExteriorContext) -> torch.Tensor:
    """Return candidate atom pairs whose expanded spheres intersect."""

    if context.num_atoms < 2:
        return torch.empty((0, 2), dtype=torch.long, device=context.device)
    try:
        return _candidate_pair_indices_kdtree(context)
    except Exception:
        return _candidate_pair_indices_torch(context)


def _candidate_pair_indices_kdtree(context: SESExteriorContext) -> torch.Tensor:
    """Find candidate expanded-sphere intersections using SciPy's KD-tree."""

    from scipy.spatial import cKDTree

    coords = context.atom_coords.detach().cpu().numpy().astype(np.float64, copy=False)
    radii = context.expanded_radii.detach().cpu().numpy().astype(np.float64, copy=False)
    tree = cKDTree(coords)
    max_radius_sum = float(2.0 * radii.max())
    raw_pairs = tree.query_pairs(max_radius_sum, output_type="ndarray")
    if raw_pairs.size == 0:
        return torch.empty((0, 2), dtype=torch.long, device=context.device)
    first = raw_pairs[:, 0]
    second = raw_pairs[:, 1]
    diffs = coords[first] - coords[second]
    dist_sq = np.einsum("ij,ij->i", diffs, diffs)
    radius_sums_sq = (radii[first] + radii[second]) ** 2
    radius_diffs_sq = (radii[first] - radii[second]) ** 2
    valid = (dist_sq < radius_sums_sq) & (dist_sq > radius_diffs_sq)
    pairs = raw_pairs[valid]
    if pairs.size == 0:
        return torch.empty((0, 2), dtype=torch.long, device=context.device)
    return torch.as_tensor(pairs, dtype=torch.long, device=context.device)


def _candidate_pair_indices_torch(context: SESExteriorContext) -> torch.Tensor:
...
def _packed_pair_parameters(
    ...
    *,
    ...
    for row, count_tensor in enumerate(counts):
        ...
        area_fracs = torch.frac(
            (torch.arange(count, dtype=dtype, device=device) + 0.5)
            * golden_ratio_conjugate
        )
        frac = _invert_pair_arc_cdf(area_fracs, arc_cdfs[row])
        row_chunks.append(
            torch.full((count,), row, dtype=torch.long, device=device),
        )
        theta_chunks.append(theta)
        frac_chunks.append(frac)
    if not row_chunks:
        return (
...
def _candidate_triple_indices(
    ...
    *,
    ...
    pairs_np = pair_indices.detach().cpu().numpy()
    adjacency = [set() for _ in range(num_atoms)]
    for first, second in pairs_np:
    ...
    for first in range(num_atoms):
        greater_neighbors = sorted(second for second in adjacency[first] if second > first)
        for second in greater_neighbors:
            common = adjacency[first].intersection(adjacency[second])
            for third in sorted(value for value in common if value > second):
    ...
    if not triples:
        return torch.empty((0, 3), dtype=torch.long, device=pair_indices.device)
    return torch.as_tensor(triples, dtype=torch.long, device=pair_indices.device)
...
def _deduplicate_centers(centers: torch.Tensor, tolerance: float) -> torch.Tensor:
    ...
    keys = np.round(centers_np / tolerance).astype(np.int64)
    ...
    return torch.as_tensor(keep_indices, dtype=torch.long, device=centers.device)
    ...
    for start in range(0, centers.shape[0], center_chunks):
        ...
        for local_row in range(stop - start):
            ...
            if merged.numel() > max_support_atoms:
                seed_set = {int(value.item()) for value in seed}
                nonseed = [value for value in merged.tolist() if value not in seed_set]
                nonseed_indices = torch.as_tensor(nonseed, device=centers.device)
                nonseed_residuals = residuals[local_row, nonseed_indices]
                order = torch.argsort(nonseed_residuals)
                keep_nonseed = torch.as_tensor(nonseed, dtype=torch.long, device=centers.device)[
                    order[: max_support_atoms - 3]
                ]
...
def _convex_hull_normal_order(normals: torch.Tensor, axis: torch.Tensor) -> torch.Tensor:
    ...
    order = sorted(range(points.shape[0]), key=lambda idx: (points[idx, 0], points[idx, 1], idx))
    if len(order) <= 3:
        return torch.as_tensor(order, dtype=torch.long, device=normals.device)

    def cross(origin: int, first: int, second: int) -> float:
        first_vec = points[first] - points[origin]
        second_vec = points[second] - points[origin]
        return float(first_vec[0] * second_vec[1] - first_vec[1] * second_vec[0])
...
def _deduplicate_weight_rows(
    ...
    *,
    ...
    keys = torch.round(weights.detach().cpu() / tolerance).to(torch.long)
    seen: set[tuple[int, ...]] = set()
    keep_indices = []
    for index, key_values in enumerate(keys.tolist()):
        key = tuple(int(value) for value in key_values)
        if key in seen:
            continue
        seen.add(key)
        keep_indices.append(index)
    return weights[torch.as_tensor(keep_indices, dtype=torch.long, device=weights.device)]
...
def _empty_samples(context: SESExteriorContext, max_support: int) -> SESAnalyticSamples:
...
def _concat_samples(
    samples: tuple[SESAnalyticSamples, ...],
    *,
    context: SESExteriorContext,
...
__all__ = [
    ...
    "build_ses_exterior_context",
    "extract_atom_contact_blocks",
    "extract_pair_torus_blocks",
    "extract_probe_sphere_blocks",
