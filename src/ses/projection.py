...
_GRID_FIRST_ACCESSIBLE_CENTER_THRESHOLD = 4096
...
def sample_projected_points(
    ...
    *,
    ...
    with torch.no_grad():
        finite_mask = torch.isfinite(projected_points.detach()).all(dim=-1)
...
def _compute_active_set_probe_centers(
    ...
    atom_ext_radii = atom_radii + probe_radius
    with torch.no_grad():
        local_atom_indices = _build_local_atom_indices(atom_coords.detach())
    ...
    block_rows = 32768 if device.type == "cuda" else 8192
    ...
    for start in range(0, total_rows, block_rows):
        ...
        with torch.no_grad():
            detached_points = block_points.detach()
            detached_coords = atom_coords.detach()
            detached_radii = atom_radii.detach()
            detached_ext_radii = atom_ext_radii.detach()

            owner_coords = detached_coords[owner_indices]
            owner_ext_radii = detached_ext_radii[owner_indices]
            local_indices = local_atom_indices[owner_indices]
            local_coords = detached_coords[local_indices]
            local_radii = detached_radii[local_indices]
            local_ext_radii = detached_ext_radii[local_indices]

            owner_directions = detached_points - owner_coords
            ...
            selected_kind = torch.zeros((row_count,), dtype=torch.long, device=device)
            selected_first = owner_indices.clone()
            selected_second = owner_indices.clone()
            selected_third = owner_indices.clone()
            selected_sign = torch.zeros((row_count,), dtype=torch.long, device=device)
            best_scores = torch.where(
                point_is_valid & direct_is_feasible,
                torch.zeros((row_count,), dtype=dtype, device=device),
                torch.full((row_count,), float("inf"), dtype=dtype, device=device),
            )
            ...
            if unresolved_indices.numel() > 0 and max_active_neighbors > 0:
                unresolved_points = detached_points[unresolved_indices]
                ...
                if pair_combinations.numel() > 0:
                    ...
                    first_coords = detached_coords[first_indices]
                    second_coords = detached_coords[second_indices]
                    first_radii = detached_ext_radii[first_indices]
                    second_radii = detached_ext_radii[second_indices]
                    ...
                    pair_mask = pair_valid & pair_feasible & pair_membership
                    masked_pair_scores = torch.where(
                        pair_mask,
                        pair_scores,
                        torch.full((), float("inf"), dtype=dtype, device=device),
                    )
                ...
                triple_rows = (
                    (~unresolved_best_scores.isfinite())
                ...
                if triple_rows.numel() > 0 and triple_combinations.numel() > 0:
                    ...
                    flat_scores = triple_scores.reshape(triple_centers.shape[0], -1)
                    flat_mask = (
                        distinct_triples
                        & triple_valid
                        & triple_feasible
                    ...
                    masked_triple_scores = torch.where(
                        flat_mask,
                        flat_scores,
                        torch.full((), float("inf"), dtype=dtype, device=device),
                    )
        ...
        best_centers = (
            owner_coords + owner_ext_radii.unsqueeze(-1) * owner_directions
        ).clone()
        ...
        if triple_rows.numel() > 0:
            triple_centers, _ = _compute_triple_probe_centers(
                atom_coords,
                atom_ext_radii,
                selected_first[triple_rows],
...
def _build_atom_cell_table(
    ...
    cache_key = (_tensor_cache_token(atom_coords), float(cell_size))
    cached = _ATOM_CELL_TABLE_CACHE.get(cache_key)
    if cached is not None:
        _ATOM_CELL_TABLE_CACHE.move_to_end(cache_key)
    ...
    _ATOM_CELL_TABLE_CACHE[cache_key] = table
    _ATOM_CELL_TABLE_CACHE.move_to_end(cache_key)
    while len(_ATOM_CELL_TABLE_CACHE) > _ATOM_CELL_TABLE_CACHE_SIZE:
        _ATOM_CELL_TABLE_CACHE.popitem(last=False)
    return table
...
def _grid_accessible_query_centers(
    query_centers: torch.Tensor,
    atom_coords: torch.Tensor,
    expanded_atom_radii: torch.Tensor,
    expanded_atom_radii_sq: torch.Tensor,
    ...
    """Check center accessibility through the cached exterior grid first."""

    if query_centers.shape[0] == 0:
        return torch.empty((0,), dtype=torch.bool, device=query_centers.device)

    reachable_grid, bbox_min, grid_spacing, dims = _external_reachable_grid(
        atom_coords,
        expanded_atom_radii,
        query_centers,
        grid_spacing,
    ...
    frac_indices = (query_centers - bbox_min) / grid_spacing
    base_indices = torch.floor(frac_indices).to(torch.long)
    dims_long = dims.to(dtype=torch.long)
    grid_accessible = torch.zeros(
        query_centers.shape[0],
        dtype=torch.bool,
        device=query_centers.device,
    )
    zero_corner = torch.zeros((1, 3), dtype=torch.long, device=query_centers.device)

    for dx in (0, 1):
        for dy in (0, 1):
            for dz in (0, 1):
                offsets = torch.tensor([dx, dy, dz], dtype=torch.long, device=query_centers.device)
                ...
                if unresolved_indices.numel() == 0:
                    continue
                corner_points = bbox_min + corner_indices[unresolved].to(query_centers.dtype) * grid_spacing
                segment_clear = _segment_clearance_mask(
                    query_centers[unresolved],
                    corner_points,
                    atom_coords,
                    expanded_atom_radii_sq,
                )
...
def _probe_centers_accessible_from_exterior(
    ...
    use_grid_first = (
        feasible_query_centers.shape[0] >= _GRID_FIRST_ACCESSIBLE_CENTER_THRESHOLD
        and calc_atom_coords.shape[0] >= 64
    )
    if use_grid_first:
        query_accessible = _grid_accessible_query_centers(
    ...
    if use_grid_first:
        molecule_center = calc_atom_coords.mean(dim=0, keepdim=True)
        exterior_dirs = unresolved_query_centers - molecule_center
        exterior_dir_norms = torch.linalg.norm(exterior_dirs, dim=-1, keepdim=True)
        fallback_dirs = torch.zeros_like(exterior_dirs)
        fallback_dirs[:, 0] = 1
        exterior_dirs = torch.where(
        ...
        exterior_points = molecule_center + exterior_dirs * exterior_radius
        segment_accessible = _segment_clearance_mask(
...
def project_points(
    ...
    with torch.no_grad():
        accessibility_mask = _probe_centers_accessible_from_exterior(
        ...
        valid_point_mask = valid_point_mask & accessibility_mask
    ...
    projected_points_on_ses = (
        ...
        * (
            probe_radius
            / point_probe_center_dists.clamp_min(torch.finfo(points.dtype).eps)
        ).unsqueeze(-1)
    )
